...
function buildVoicePicker(voices, best) {
  ...
  relevant.forEach(v => {
    ...
    opt.value = v.name;
    opt.textContent = (v.name === best?.name ? '✅ ' : '') + v.name + ' (' + v.lang + ')';
    if (v.name === best?.name) opt.selected = true;
    sel.appendChild(opt);
  });
}
...
function saveState() {
  try {
    localStorage.setItem('fl_user',    STATE.user);
    localStorage.setItem('fl_mascot',  STATE.mascot);
    localStorage.setItem('fl_stars',   String(STATE.totalStars));
    localStorage.setItem('fl_correct', String(STATE.correctCount));
    localStorage.setItem('fl_visited', String(STATE.sectionsVisited));
    localStorage.setItem('fl_draw',    String(STATE.drawings));
    localStorage.setItem('fl_vset',    JSON.stringify([...STATE.visitedSet]));
    localStorage.setItem('fl_badges',  JSON.stringify([...STATE.earnedBadges]));
  } catch (e) { /* storage unavailable */ }
}

function loadState() {
  try {
    STATE.user            = localStorage.getItem('fl_user')    || '';
    STATE.mascot          = localStorage.getItem('fl_mascot')  || '🦄';
    STATE.totalStars      = parseInt(localStorage.getItem('fl_stars')   || '0', 10);
    STATE.correctCount    = parseInt(localStorage.getItem('fl_correct') || '0', 10);
    STATE.sectionsVisited = parseInt(localStorage.getItem('fl_visited') || '0', 10);
    STATE.drawings        = parseInt(localStorage.getItem('fl_draw')    || '0', 10);
    const vs = JSON.parse(localStorage.getItem('fl_vset')   || '[]');
    const bs = JSON.parse(localStorage.getItem('fl_badges') || '[]');
...
function initLogin() {
  ...
  // Pre-fill saved name
  if (STATE.user) nameInput.value = STATE.user;
...
function doLogin() {
  ...
  setTimeout(() => speakText(
    'Good ' + tod + ' ' + name + '! Welcome to Fun Learn! I am so excited to learn with you today! Tap anything to start!',
    true
  ), 600);
}
...
function initVoicePanel() {
  ...
  if (testBtn) testBtn.addEventListener('click', () => speakText('Hello! I am your Fun Learn teacher! Let us learn together!', true));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Greet on login page
  setTimeout(() => speakText('Hello! Welcome to Fun Learn! Please type your name and tap the Let us Go button!', true), 800);
});
